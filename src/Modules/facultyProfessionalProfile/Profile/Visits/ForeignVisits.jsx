...
//     try {
//       const response = await axios.get(
//         "http://127.0.0.1:8000/eis/api/fvisits/pf_no/", {
//           params: { pfNo },
//         }
//       );
//       const projects = response.data;
...
//       if (isEdit === false) {
//         const res = await axios.post(
//           "http://127.0.0.1:8000/eis/api/fvisit/",
//           formData,
//         );
//         console.log(res.data);
//       } else {
...
//       try {
//         await axios.post(
//           `http://127.0.0.1:8000/eis/api/emp_visitsDelete/`,
//           new URLSearchParams({ pk: projectId }),
//         ); // Adjust the delete URL as needed
//         fetchProjects(); // Refresh the project list after deletion
//       } catch (error) {
...
export default function IndianVisits() {
  ...
  const fetchProjects = async () => {
    try {
      const response = await axios.get(
        "http://127.0.0.1:8000/eis/api/ivisits/pf_no/", {
          params: { pfNo },
        }
      );
      const projects = response.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit === false) {
        const res = await axios.post(
          "http://127.0.0.1:8000/eis/api/ivisit/",
          formData,
        );
        console.log(res.data);
      } else {
