...
export default function ConferenceSymposium() {
  ...
  const fetchProjects = async () => {
    try {
      const response = await axios.get(
        "http://127.0.0.1:8000/eis/api/consym/pf_no/", {
          params: { pfNo },
        }
      );
    ...
    try {
      ...
      if (isEdit === false) {
        const res = await axios.post(
          "http://127.0.0.1:8000/eis/api/consym/",
          formData,
        );
        ...
        const res = await axios.post(
          "http://127.0.0.1:8000/eis/api/consym/edit",
          formData,
        );
        console.log(res.data);
        setEdit(false);
  ...
  const handleDelete = async (projectId) => {
    ...
    if (
      ...
      try {
        await axios.post(
          `http://127.0.0.1:8000/eis/api/emp_consymDelete/`,
