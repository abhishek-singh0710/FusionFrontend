...
export default function Conference() {
  ...
  const fetchConferences = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:8000/eis/api/fetch_conference/", {
        params: { pfNo },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        await axios.post("http://127.0.0.1:8000/eis/api/conference/edit", formData);
      } else {
        const formData = new FormData();
        formData.append("author", inputs.author);
        formData.append("co_authors", inputs.coAuthor);
        ...
        formData.append("dop", inputs.dateOfPublication);
        formData.append("dos", inputs.dateOfSubmission);
        await axios.post("http://127.0.0.1:8000/eis/api/conference/", formData);
      }
      fetchConferences();
      setInputs({
        author: "",
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.post("http://127.0.0.1:8000/eis/api/emp_confrence_organisedDelete/", formData);
      fetchConferences();
    } catch (error) {
